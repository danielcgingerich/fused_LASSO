...
*Updating* $\beta$ &mdash; We differentiate the augmented Lagrangian with respect to $\beta$ and set it equal to zero. This results in the following linear system: 
...
*Update* $z$ *and* $d$ &mdash; At the $k$-th step, $z^{(k+1)}$ and $d^{(k+1)}$ are updated as follows: 
